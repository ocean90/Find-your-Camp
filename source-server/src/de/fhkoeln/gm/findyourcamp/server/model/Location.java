...
public class Location {
	...
	public static int getIdByName(String locationName) {
		...
		try {
			...
			ResultSet locationResultSet = statement.executeQuery(
				"SELECT " + LocationsTable.COLUMN_NAME_LOCATION_ID + " FROM " +
				LocationsTable.TABLE_NAME + " WHERE " +
				LocationsTable.COLUMN_NAME_LOCATION_NAME + "='" +
				locationName + "'"
			);
	...
	public static int createLocation(String rentalPropertyLocation) {
		...
		try {
			...
			statement.executeUpdate(
				"INSERT INTO "+ LocationsTable.TABLE_NAME +
				" (" + LocationsTable.COLUMN_NAME_LOCATION_NAME + ") " +
				"VALUES ( '" + rentalPropertyLocation + "');", Statement.RETURN_GENERATED_KEYS
			);
