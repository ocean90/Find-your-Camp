...
public class MessageBroker {
	...
	public MessageBroker(Map<String, Object> object) {
		...
		try {
			this.data = (Map<String, Object>) JSONValue
	...
	private void handleRentalPropertyRegistration() {
		...
		long rentalPropertyLocalId = (Long) data
				.get("rental_property_local_id");
		String rentalPropertyLocation = (String) data
				.get("rental_property_location");

		// Neue ID nach Anlegen erhalten
		long newRentalPropertyId = RentalProperty.createRentalPropertyEntry(
				rentalPropertyLocalId, rentalPropertyLocation, userId);
		...
		System.out.println("Registrierung für Mietobjekt "
				+ rentalPropertyLocalId + "(Remote: " + newRentalPropertyId
				+ ") abgeschlossen.");
	}
	...
	public void handleUserRegistration() {
		...
		if (newUserId > 0) {
			...
			System.out.println("Registrierung für User " + newUserId
					+ " und Device " + newDeviceId + " abgeschlossen.");
	...
	public void handleSearchRequest() {
		...
		HashMap<String, ArrayList<Object>> matches = LocationMatch
				.getMatches(location);
		ArrayList<Object> registrationIds = matches.get("registrationIds");
		ArrayList<Object> localRentalPropertyIds = matches
				.get("localRentalPropertyIds");
		...
		if (registrationIds.isEmpty()) {
			...
			for (Iterator<Object> registrationIdsIterator = registrationIds
					.iterator(); registrationIdsIterator.hasNext();) {
				for (Iterator<Object> localRentalPropertyIdsIterator = localRentalPropertyIds
						.iterator(); localRentalPropertyIdsIterator.hasNext();) {

					String registrationId = (String) registrationIdsIterator
							.next();
					Integer localRentalPropertyId = (Integer) localRentalPropertyIdsIterator
							.next();
					...
					payload.put("rental_property_local_id",
							localRentalPropertyId);
					message.setPayload(payload);
					gcmConnection.sendMessage(message);
				}
			}
		...
		System.out.println("Suche nach " + location + " ergab "
				+ registrationIds.size() + " Treffer.");
	}
}
